...
{
	Key * parentKey = keyNew ("user/tests/prettyexport", KEY_VALUE, "/home/rene/table.rst", KEY_META, "description",
    ...
    printf("%s\n", keyString(parentKey));
	//succeed_if (compare_line_files (srcdir_file ("prettyexport/basicTable.rst"), keyString (parentKey)), "files do not match as expected");
	ksDel (ks);
	keyDel (parentKey);
	PLUGIN_CLOSE ();
}
