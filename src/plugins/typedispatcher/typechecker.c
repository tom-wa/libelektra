...
static int doTypeCheck (DispatchConfig * config, TypeConfig * tc, ArgumentConfig * argConfig, const Key * key, KeySet * returned)
{
	ksRewind (tc->checks);
	Key * checkMeta;
	...
	while ((typeMeta = ksNext (tc->types)) != NULL)
	{
		...
		RC r = doTypeCheck (config, t_tc, ac, key, returned);
...
static int typeCheck (DispatchConfig * config, const Key * key, KeySet * returned, const char * typeString, Key * parentKey ELEKTRA_UNUSED)
{
#ifdef DEVBUILD
	...
	RC rc = doTypeCheck (config, tc, argConfig, key, returned);
